        ...
        if (con != null) {
            try {
                if (newStatus == null || newStatus.isEmpty()) {
                    PreparedStatement p1 = con.prepareStatement("SELECT status FROM du_reg WHERE id = ?");
                    p1.setString(1, id);
                ...
                PreparedStatement p2 = con.prepareStatement("UPDATE du_reg SET status = ? WHERE id = ?");
                p2.setString(1, newStatus);
                p2.setString(2, id);
                int i = p2.executeUpdate();
                if (i != 0) {
                    response.sendRedirect("dataUsers.jsp?Updated");
                } else {
