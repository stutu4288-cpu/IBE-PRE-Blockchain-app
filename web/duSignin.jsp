<%-- 
    Document   : du_signin (Security Hardened)
...
<%
    ...
    if (mail == null || pass == null) {
        response.sendRedirect("duLogin.jsp?Failed"); return;
    }
    ...
    if (con == null) { response.sendRedirect("duLogin.jsp?DB_Error"); return; }

    try {
        // --- Brute-force lockout: count FAILED attempts in last 10 minutes ---
        PreparedStatement chk = con.prepareStatement(
            "SELECT COUNT(*) FROM login_log WHERE ip_address=? AND user_type='DU' AND status='FAILED' AND login_time > DATE_SUB(NOW(), INTERVAL 10 MINUTE)");
        chk.setString(1, ip);
        ResultSet rchk = chk.executeQuery();
        rchk.next();
        int attempts = rchk.getInt(1);
        if (attempts >= 5) {
            response.sendRedirect("duLogin.jsp?Locked");
            return;
        }

        // --- Step 1: PreparedStatement login query (matching email/phone/name) ---
        PreparedStatement ps = con.prepareStatement(
            "SELECT * FROM du_reg WHERE email=? OR phone=? OR name=?");
        ...
        if (rs.next()) {
            ...
            if (!"Approved".equalsIgnoreCase(status)) {
                if ("Revoked".equalsIgnoreCase(status)) {
                    response.sendRedirect("duLogin.jsp?msg=Account_Revoked");
                } else {
                    response.sendRedirect("duLogin.jsp?msg=Account_Pending");
                }
                return;
            }
            ...
            if (passMatches && pkeyMatches) {
                ...
                session.setAttribute("dupkey", dbPKey);

                // Log success
                PreparedStatement log = con.prepareStatement(
                    "INSERT INTO login_log(user_type,user_id,email,ip_address,status) VALUES('DU',?,?,?,'SUCCESS')");
                ...
                PreparedStatement log = con.prepareStatement(
                    "INSERT INTO login_log(user_type,user_id,email,ip_address,status) VALUES('DU', ?, ?, ?, 'FAILED')");
        ...
        ex.printStackTrace();
        response.sendRedirect("duLogin.jsp?Error");
    }
%>