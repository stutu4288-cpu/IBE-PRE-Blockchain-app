...
def get_connection():
    """Returns an active DBConnection (MySQL primary if configured & reachable, otherwise clean SQLite)."""
    has_mysql_env = bool(os.environ.get("MYSQLHOST") or os.environ.get("MYSQL_HOST") or os.environ.get("MYSQL_URL") or os.environ.get("DATABASE_URL"))
    
    if HAS_PYMYSQL and has_mysql_env:
        try:
            ...
            pass  # Silently fallback to SQLite if MySQL is not ready/reachable
